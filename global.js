...
//    $('.table-list tr:odd').addClass('alt');
    function getSoDongTr(tr){
        var sodong = 0;
        for (var i = 0; i < tr.length; i++) {
            var td = tr[i];
            if($(td).attr('colspan')){
                sodong += $(td).attr('colspan');
            }else{
                sodong++;                
            }
        }
    ...
    function soDongMax(rows){
        var sodong = 0;
        $(rows).each(function(){
            var sodong_tr = getSoDongTr($(this).children());
            if(sodong < sodong_tr){
                sodong = sodong_tr;
            }
        });
    ...
    $('.table-list').each(function(){
        var rows = $(this).find("tbody").children();
        var sodong = soDongMax(rows);
    ...
    $(".table-list td,th").hover(function() {
        var td = $(this);
        var tbody = td.parent().parent();// tbody của table
        var rows = tbody.children(); // các dòng tr trong tbody
        var sodong = soDongMax(rows);
        var firstRow = findFirstRows(td,sodong);
        var sl_break = 0;
        var sodongfind = 0; // giới hạn 30 dòng tránh trường hợp vô tận
        firstRow.find('td').addClass('hover');
        firstRow.find('th').addClass('hover');
        while (sodongfind < sodong && sl_break < 30) {            
            firstRow = firstRow.next();
            sodongfind = getSoDongTr(firstRow.children());
            if(sodongfind < sodong){                
                firstRow.find('td').addClass('hover');
//                firstRow.find('th').addClass('hover');
            }
            sl_break++;
        ...
        var td = $(this);
        var tbody = td.parent().parent();// tbody của table
        tbody.find('td').removeClass('hover');
        tbody.find('th').removeClass('hover');
    });

    function findFirstRows(td, sodong) {
        //tìm dòng đầu tiên của td
        var row = td.parent();
        var firstRow = row;
        var sodongfind = getSoDongTr(row.children());
        var sl_break = 0;
        while (sodongfind < sodong && sl_break < 30) {
                row = row.prev();
                firstRow = row;
                sodongfind = getSoDongTr(row.children());
                sl_break++;
            } 
    ...
    /*-------------*/
    if ( $('#popupcontent ul').children().length == 0 ) { //nếu ko có thông báo còn hạn thì ẩn popup đi(xử lý khi có tin cũ mà ko có tin còn hạn thì sẽ hiện popup rỗng)
        $("#popup").hide(); 
    }
    
    if($("#popupcontent").hasClass('popup')){ //nếu là popup hiện giữa màn hình và có tin chưa đọc thì bật overlay
        $("#popupcontent li").each(function(){
            if($(this).hasClass('unread')){
                $('#overlay_popup').slideDown(); 
                return false; 
            }
        });
    }    
    var showAllNotice = 0;
    $("#notice").click(function(){ // khi click vào icon chuông
        if(showAllNotice == 0){ //nếu đang hiện tất cả thông báo
            $("#popup").show();
            $("#popupcontent").hide(); //ẩn khung thông báo còn hạn
            $("#noticecontent").show(); //hiện khung tất cả thông báo
            $("#popup .title").html('Tất cả thông báo');
            showAllNotice = 1;
        }
        else {
            $("#popupcontent").show();
            $("#noticecontent").hide();
            $("#popup .title").html('Thông báo từ quản trị');
            showAllNotice = 0;
        }
    ...
    $(".popupmin").click(function(){
        if(showAllNotice == 1){ //khi đang hiện tất cả thông báo
            if(tb_cookie == "min") {
                $("#noticecontent").slideDown();
                tb_cookie    = "max";
            } else {
        ...
        else {//khi đang hiện thông báo còn hạn
            if(tb_cookie == "min") {
    ...
    $(".popupclose").click(function(){
        if(showAllNotice == 1){ //khi đang hiện tất cả thông báo
            ...
            //kiểm tra xem có thông báo chưa đọc ko
            $readall = true;
            $("#popupcontent li").each(function(){
                if($(this).hasClass('unread')){
                    $readall = false;
            ...
            if(!$readall){
                //alert('Vẫn còn thông báo từ quản trị chưa đọc.');
                // dpqduong - 01/03/2018
                alert('Vui lòng đọc tất cả các thông báo trước khi đóng.');
            }
            else {
    ...
    //14/06/2017: pqbao
    $("select option[value='dgrl_key_parent']").attr('disabled', true );
    //
    if (requreChangePass) {
        showModalProfile('dlg_profile',root_url+'/ajax/showprofile');
    } else {
        $("#warning_changepass").hide();
    }
    //nchien - 27/02/2018
    $("[name^='multiple_']").each(function(){
        $(this).multiselectfilter({
            label: "Tìm:",
                    width: '180', /* override default width set in css file (px). null will inherit */
...
function showModalNotice(id, url) { 
    ...
    $('#'+id).show('puff');
    /***/
    // dpqduong -02/03/2018 Ẩn Modal thông báo chính khi click vào thông báo
    $('#popup').hide();
    showLoading();
    console.log(url);
    $.ajax({
...
function hideModal(id) {
    ...
    sys_close_form = true;
    $('#'+id).hide('puff');
    // Hiển thị lại Modal Thông báo chính
    $('#popup').show();
}
    ...
    if (old >= 0) {
        for (var i = old + 1; i < old + order.length; i++) {
            ...
            if (document.getElementById(order[j] + name) && document.getElementById(order[j] + name).style.display == "none") {
                ...
                if($('#h_type_loopkup_' + name).val()){
                    document.getElementById('h_type_loopkup_' + name).value = order[j].substring(0, order[j].length - 1);
                }
...
function processCheckAll(name) {
    ...
    if($('#chk_' + name + '_check_all_top').val()){
        document.getElementById('chk_' + name + '_check_all_top').checked = b;
        if ($('#chk_' + name + '_check_all_top').is(":checked")) {
            $('#chk_' + name + '_check_all_top').parent().addClass("selected");
        } else {
            $('#chk_' + name + '_check_all_top').parent().removeClass("selected");
        }
    }
}

function processCheckAllTop(name) {
    var chk = document.getElementsByName('chk_' + name + '_all');
    var b = document.getElementById('chk_' + name + '_check_all_top').checked;
    for (var i = 0; i < chk.length; i++) {
        chk[i].checked = b;
        if($(chk[i]).is(":checked")) {
            $(chk[i]).parent().addClass("selected");
    ...
    document.getElementById('chk_' + name + '_check_all').checked = b;
    if ($('#chk_' + name + '_check_all').is(":checked")) {
        $('#chk_' + name + '_check_all').parent().addClass("selected");
    } else {
        $('#chk_' + name + '_check_all').parent().removeClass("selected");
    }
}
...
//            $(obj).click();
            ...
            var id = '#' + obj.id;
            document.getElementById(obj.id).parentNode.innerHTML = document.getElementById(obj.id).parentNode.innerHTML;
            $(id).click();            
...
//        }
    ...
    sys_lookup_type = $('#h_type_loopkup_' + obj).val();
    var giatri = '';
    if(sys_lookup_type == 'multiple'){
        giatri = JSON.stringify($('#' + sys_lookup_type +'_'+ obj).val());
    }
    $.ajax({
        type: "GET",
        url: root_url + "/index.php",
        data: "rt=ajax/public_lookup&key=" + key + "&value="+"&where="+ strwhere +"&obj="+obj+"&offsets="+offset+"&where_string="+sys_lookup_where+"&type=" + sys_lookup_type+"&giatri=" + giatri,
        success: function(rXML){
            if (!rXML) {
                alert("Failed to connect.");
            }
            ...
            document.getElementById('div_footer_lookup').innerHTML = $(rXML).find('button').text();
...
function showNextLookup(obj, key, where, offset) {
    ...
    //dthung - 17/12/2019 Bổ sung thêm param vào ajax search như hàm getLookup()
    var stringAddLookup = "";
    var dsInputName = "";
...
//        url: "index.php",
        url: root_url + "/index.php",
        data: "rt=ajax/public_lookup&key=" + key + "&value="+valuesearch+"&where="+ strwhere +"&obj="+obj+"&offsets="+offset+"&&where_string="+sys_lookup_where+stringAddLookup+"&type=" + sys_lookup_type,
        success: function(rXML){
            if (!rXML) {
                alert("Failed to connect.");
            }
...
//        url: "index.php",
        url: root_url + "/index.php",
        data: "rt=ajax/public_lookup&key=" + sys_lookup_key + "&value=" + encodeURIComponent(document.getElementById('txt_lookup_value').value)+"&where="+strwhere+"&obj="+sys_lookup_desc+"&offsets=0&where_string="+sys_lookup_where+stringAddLookup+"&type=" + sys_lookup_type,
        success: function(rXML){
            if (!rXML) {
                alert("Failed to connect.");
            }
...
//                    document.getElementById(id).checked = true;
    ...
    if ($("#div_lookup input[type=checkbox]") != null) {
        $.each($("#div_lookup input[type=checkbox]"), function (k, v) {
            if($(v).is(":checked")) {
                $(v).parent().addClass("selected");
            } else {
                $(v).parent().removeClass("selected");
...
//        $("#div_lookup input[type=checkbox]").click(function(){
//            var id = $(this).attr('id');
//            if($(this).is(":checked")) {
////                document.getElementById(id).checked = false;
//                    $('#'+id).parent().addClass("selected");
...
function selectLookup(ind) {
    ...
    hideModal('dlg_frmLookup');console.log('#h_lst_lookup_' + ind);
    ...
    $('#'+sys_lookup_desc+'').val($('#h_lst_lookup_' + ind).val());
//    if($('#multiple_' + sys_lookup_desc).val()){
//        var selected = [$('#h_lst_lookup_' + ind).val()];
//        checkMultiSelect('multiple_' + sys_lookup_desc,selected);
//    }
    ...
    $('#txt_' + sys_lookup_desc).focus();
    $('#'+sys_lookup_desc+'').focus();
}

function selectLookup_chk() {
    
    hideModal('dlg_frmLookup');
    var selected = [];
    var chk = document.getElementsByName('chk_lst_lookup_chon[]');
    for (var i = 0; i < chk.length; i++) {
        if (chk[i].checked) {
            selected.push($('#h_lst_lookup_' + i).val());
        }
    }
    checkMultiSelect('multiple_' + sys_lookup_desc,selected,true);
}
...
function checkFile(fieldObj)
    ...
    function checkMultiSelect(id, selected, uncheck = false){
        var multiple = document.getElementById(id);

        for ( var i = 0; i < multiple.options.length; i++ )
        {
            var option = multiple.options[i];
